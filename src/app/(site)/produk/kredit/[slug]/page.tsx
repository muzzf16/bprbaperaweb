...
export async function generateMetadata(
    ...
    if (!product) {
        product = CREDIT_PRODUCTS.find((p) => p.slug === params.slug);
    }

    if (!product) {
        return {
            title: "Produk Tidak Ditemukan - BPR Bapera",
        };
    }
...
export default async function CreditDetailPage({ params }: Props) {
    ...
    try {
        ...
        console.error("Failed to fetch product from Sanity", e);
    }

    // Fallback to static data if not found in Sanity
    if (!product) {
        const staticProduct = CREDIT_PRODUCTS.find((p) => p.slug === params.slug);
        if (staticProduct) {
            product = {
                ...staticProduct,
                    ...
                    <div className="lg:w-1/3">
                        <div className="bg-blue-900 text-white p-8 rounded-2xl shadow-xl sticky top-24">
                            ...
                            <div className="space-y-6">
                                ...
                                {product.minTenor && product.maxTenor && product.minTenor !== "-" && (
                                    <div className="flex items-start">
